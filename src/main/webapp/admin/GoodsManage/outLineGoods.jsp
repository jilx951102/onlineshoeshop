...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe620;</i>商品管理<span class="c-666 en">&gt;</span>下架商品列表
...
<div class="col-7 col-xs-offset-3"  style="margin-top: 10px">
   ...
   <input type="text" id="shoeName" value="${shoeName }" placeholder="输入商品名" class="input-text radius size-M" style="width: 280px">
...
<div class="col-12" style="margin-top: 20px">
   <button type="button" onclick="shoeDelAll()" class="size-M btn btn-danger radius"><i class="Hui-iconfont">&#xe60b;</i>删除</button>
   <button type="button" onclick="shoeOnAll()" class="size-M btn btn-primary radius"><i class="Hui-iconfont">&#xe6dc;</i>上架</button>
   <button type="button" onclick="location.href='shoe_exportExcel2'" class="size-M btn btn-primary radius"><i class="Hui-iconfont">&#xe644;</i>导出</button>
</div>
          ...
          <c:forEach items="${page.shoeList}" var="s" varStatus="vst">
             <tr>
		        ...
		        <td class="text-c">
		           ...
		           <a href="javascript:;" onclick="shoeOn(this)" ><span class="Hui-iconfont" style="font-weight:normal;">&#xe6dc;上架</span></a>
		           <a href="javascript:;" onclick="onShoeEdit(this)" ><span class="Hui-iconfont" style="font-weight:normal;">&#xe60c;修改</span></a>
		           <a href="javascript:;" onclick="shoeDel(this)" ><span class="Hui-iconfont" style="font-weight:normal;">&#xe60b;删除</span></a>
		        </td>
		      </tr>
		     </c:forEach>
         </c:when>
...
layui.use(['util', 'laydate', 'layer','laypage'], function(){
		    ...
		    jump: function(obj,first){
		         if(!first){
		        	 var typeId = $("#typeId").val();
						...
						if(ss==="1"){
							location.href = "shoe_searchOutLineShoe?typeId="+typeId+"&shoeName="+shoeName+"&ss="+ss+"&currentPage="+obj.curr;
						}else{
							location.href = "shoe_initShoeOutline?typeId="+typeId+"&shoeName="+shoeName+"&currentPage="+obj.curr;
						}
		         }
		        }
	  })
				  ...
				  success:function(data){
					  if(data===1){
						  layer.msg("下架成功",{
							...
							var shoeName = $("#shoeName").val();
							if(ss==="1"){
								location.href = "shoe_searchOutLineShoe?typeId="+typeId+"&shoeName="+shoeName+"&ss="+ss+"&currentPage="+currentPage;
							}else{
								location.href = "shoe_initShoeOutline?typeId="+typeId+"&shoeName="+shoeName+"&currentPage="+currentPage;
							} 
						  });
					  }else{
						  layer.alert("下架失败",{
  ...
  function searchShoe(){
	  ...
	  location.href = "shoe_searchOutLineShoe?typeId="+typeId+"&shoeName="+shoeName+"&ss="+1;
  }
  function shoeDelAll(){
